...
/**
 * 供应商客户维护表(customer_supply)表控制层
 *
 * @author makejava
 * @since 2024-02-26 20:37:53
 */
@RestController
@RequestMapping("customerSupply")
@Api(tags="供应商客户维护表")
public class CustomerSupplyController {
    /**
     * 服务对象
     */
    @Resource
     ...
     * @return 查询结果
     */
    @PostMapping("/queryCustomerSupplyPageList")
    @ApiOperation("查询列表(分页)")
    public QueryByPageOutDTO queryByPage(@RequestBody QueryByPageDTO queryByPageDTO) {
        QueryByPageOutDTO outDTO = this.customerSupplyService.queryByPage(queryByPageDTO);
        return outDTO;
    }
    ...
    public QueryByIdOutDTO queryById(@RequestBody QueryByIdDTO dto) {
        QueryByIdOutDTO outDTO = this.customerSupplyService.queryById(dto);
        return outDTO;
    }
    ...
    public AddOutDTO add(@RequestBody AddDTO dto) {
        AddOutDTO outDTO = customerSupplyService.insert(dto);
        return outDTO;
    }
    ...
    public EditOutDTO edit(@RequestBody EditDTO dto) {
        EditOutDTO outDTO = customerSupplyService.update(dto);
        return outDTO;
    }
    ...
    public DeleteByIdOutDTO deleteById(@RequestBody DeleteByIdDTO dto) {
        DeleteByIdOutDTO outDTO = customerSupplyService.deleteById(dto);
        return outDTO;
    }
     ...
     */
    @PostMapping("/export")
    @ApiOperation("导出供应商客户列表")
    public void export(@RequestBody QueryByPageDTO dto) {
        List<CustomerSupplyInfo> list = customerSupplyService.queryListForExport(dto);
        ExcelUtil.exportExcel(list, "供应商客户", CustomerSupplyInfo.class);
    }
