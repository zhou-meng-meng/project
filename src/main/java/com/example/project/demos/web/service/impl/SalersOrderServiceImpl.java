        ...
        try{
            ...
            if(CollectionUtil.isNotEmpty(userList) && userList.size() > 0){
                ...
                for(SysUserEntity userEntity : userList){
                    ApproveOperationQueueEntity queueEntity = new ApproveOperationQueueEntity(null,flowEntity.getId(), entity.getId(),FunctionTypeEnums.SALERS_ORDER.getCode(),userEntity.getUserLogin(),dto.getCustomerCode(),dto.getMaterialCode(),dto.getLoadNum(),user.getUserLogin(),date,Constants.SYSTEM_CODE,dto.getLoadDate());
