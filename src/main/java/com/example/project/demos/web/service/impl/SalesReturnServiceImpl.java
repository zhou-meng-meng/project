...
public class SalesReturnServiceImpl  implements SalesReturnService {
        ...
        try{
            ...
            if(CollectionUtil.isNotEmpty(userList) && userList.size() > 0){
                ...
                for(SysUserEntity userEntity : userList){
                    ApproveOperationQueueEntity queueEntity = new ApproveOperationQueueEntity(null,flowEntity.getId(), entity.getId(),FunctionTypeEnums.SALES_RETURN.getCode(),userEntity.getUserLogin(),dto.getCustomerCode(),dto.getMaterialCode(),dto.getReturnCount(),user.getUserLogin(),date,Constants.SYSTEM_CODE,dto.getReturnTime());
    ...
    public int updateApprove(Long id, String result, String opinion, String userLogin, BigDecimal unitPrice, BigDecimal tollAmount, Date date,String inCode,BigDecimal freight)  {
        ...
        if(result.equals(ApproveConfirmResultEnums.APPROVE_CONFIRM_RESULT_AGREE.getCode())){
            ...
            log.info("生成往来账信息");
            AddPayBySystemDTO dto = new AddPayBySystemDTO(null, entity.getId(), entity.getInCode(),entity.getBillNo(),entity.getCustomerCode(), entity.getMaterialCode(), entity.getReturnTime(),SysEnums.SYS_NO_FLAG.getCode(),entity.getCreateBy(),date,entity.getRemark(), entity.getUnitPrice(),entity.getReturnCount(),tollAmount);
